...
def greedy_decode(manager, src_encs, src_mask, max_length=512):
    ...
    for i in range(1, max_length):
        ...
        scores = model.generator(tgt_encs[:, -1, :], decoding_size)
...
def beam_decode(manager, src_encs, src_mask, beam_size, max_length=512):
    ...
    while (i := i + 1) < max_length and beam_size > 0:
        ...
        frontier = model.generator(tgt_encs[:, -1, :], decoding_size)
        ...
        paths[active] = paths[active][topi // decoding_size]
        paths[active, i], probs[active] = topi % decoding_size, topv
