...
def train_epoch(manager, criterion, optimizer=None, scalar=None, use_tqdm=False, *, mode='train'):
...
def train_model(manager, logger, use_tqdm=False):
    ...
    for epoch in range(manager.max_epochs):
        ...
        train_loss = train_epoch(manager, criterion, optimizer, scalar, use_tqdm)
...
def main():
    ...
    parser.add_argument('--save', metavar='FILE', required=True, help='save model')
    ...
    with open(args.vocab) as vocab_file, open(args.codes) as codes_file, \
        ...
        manager = Manager(src_lang, tgt_lang, vocab_file, codes_file,
            args.save, config, device, data_file, test_file)
    ...
    logger.addHandler(logging.FileHandler(args.save + '.log'))
